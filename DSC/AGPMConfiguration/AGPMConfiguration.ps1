<#
    .SYNOPSIS
        This is the Configuration for AGPM
    .DESCRIPTION
        The AGPM application requires specific features to be available and folders to be
        present, this DSC accomodates that. It is based off the default set of features
        that are present in a stock Windows Server 2012 build, some additional features
        are enabled by ITES and they are all lumped together at the top.
        Below the set of default features are the set of required features for AGPM, they
        include the following:
            ...
            WAS-NET-Environment
            WAS-Process-Model
        In addition AGPM requires a folder for Archives and that the Windows\TEMP folder 
        be available, those are listed in the two FILE resources below the set of 
        required features.
        AGPMArchive is a variable that is passed in when the configuration is executed 
        for the first time.
    .PARAMETER ComputerName
        The name of the computer that the configuration will apply to
    .PARAMETER AGPMArchive
        The archive path of the folder to create if it doesn't exist
    .NOTES
        This configuration requires no special modules in order to run
        You may need to run the following first, make sure the source is available
        dism /online /enable-feature /featurename:NetFX3 /all /Source:d:\sources\sxs /LimitAccess
    .EXAMPLE
        . .\Defaults.ps1
        AGPMConfiguration -ComputerName agpm-server -AGPMArchive 'C:\Archive'
        Start-DscConfiguration agpm-server
#>
Configuration AGPMConfiguration
{
    param
    (
    ...
    {
        #
        #region These are the default on features for ITES
        #
        ...
        #endregion
        #region These features are required for AGPM
        #
        ...
        #endregion
        #region These folders are required for AGPM
        #
        File ArchiveFolder
        {
            DestinationPath = $AGPMArchive
            Type = "Directory"
            Ensure = "Present"
            }
        File WindowsTEMP
